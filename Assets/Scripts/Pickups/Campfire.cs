...
public class Campfire : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.CompareTag("Enemy"))
        {
            ...
            if (enemy != null)
            {
                enemy.TakeDamage(10f); // O el da�o que quieras para los enemigos
            }
        }
        else if (other.CompareTag("Tear"))
        {
            ExtinguishFire();
            Destroy(other.gameObject); // Destruir la l�grima tras impacto
        }
    }

    void ExtinguishFire()
    {
        if (!hasFire) return; // Si ya est� apagada, no hacer nada
